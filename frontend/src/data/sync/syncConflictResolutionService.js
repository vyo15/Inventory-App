...
const resolveLocalWins = async (conflict, { timestamp }) => {
  ...
  assertNoSensitiveLocalDbPayload(conflict.localPayload, "conflict local payload");
  ...
  // IMS NOTE [AKTIF/GUARDED]: local_wins harus menutup status dirty local juga.
  // Jika hanya queue yang dibuat synced, record categories/customers tetap terlihat pending
  // di Offline Mode/Health Audit walaupun Firebase sudah menerima payload local.
  const dbLocal = getImsLocalDb();
  const existingLocalRecord = await dbLocal.table(conflict.collectionName).get(conflict.documentId);
  await dbLocal.table(conflict.collectionName).put({
    ...
    updatedAt: conflict.localPayload?.updatedAt || existingLocalRecord?.updatedAt || timestamp,
  });
...
const resolveRemoteWins = async (conflict, { timestamp }) => {
  ...
  assertNoSensitiveLocalDbPayload(remotePayload, "conflict remote payload");
