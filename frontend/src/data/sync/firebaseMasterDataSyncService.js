...
import {
  createSyncRecordFingerprint,
  describeComparableVersion,
  getComparableVersionTime,
} from "./syncVersionGuard";

export const MASTER_DATA_SYNC_CONFIRMATION = "SYNC MASTER DATA PILOT TO FIREBASE";
export const FIREBASE_MASTER_DATA_SYNC_CONFIRMATION = MASTER_DATA_SYNC_CONFIRMATION;
export const MASTER_DATA_DELETE_SYNC_CONFIRMATION = "DELETE MASTER DATA PILOT FROM FIREBASE";
...
const SYNC_BLOCKED_COLLECTION_REASON = Object.freeze({
  ...
  [LOCAL_DB_TABLES.PRODUCTS]:
    "Product snapshot bersifat read-only. Offline → Firebase diblokir karena terkait stock, sales, report, dan HPP.",
  [LOCAL_DB_TABLES.RAW_MATERIALS]:
    "Raw Material snapshot bersifat read-only. Offline → Firebase diblokir karena terkait purchase, production, stock, dan supplier linkage.",
  [LOCAL_DB_TABLES.SEMI_FINISHED_MATERIALS]:
    "Semi Finished snapshot bersifat read-only. Offline → Firebase diblokir karena terkait production output, work log, payroll, HPP, dan stock.",
  [LOCAL_DB_TABLES.STOCK_SNAPSHOTS]:
    "Stock snapshot bersifat read-only. Offline → Firebase diblokir karena mutasi stok wajib lewat flow Firebase stock/inventory log/read model.",
  [LOCAL_DB_TABLES.PRODUCTION_PLANS]:
    "Production planning snapshot bersifat read-only. Offline → Firebase diblokir karena production flow tetap Firebase-primary.",
  [LOCAL_DB_TABLES.PRODUCTION_ORDERS]:
    "Production order snapshot bersifat read-only. Offline → Firebase diblokir karena start/finish/stock mutation tetap Firebase-primary.",
  [LOCAL_DB_TABLES.PRODUCTION_WORK_LOGS]:
    "Production work log snapshot bersifat read-only. Offline → Firebase diblokir karena konsumsi bahan, output stock, payroll, dan HPP tetap guarded.",
  [LOCAL_DB_TABLES.PRODUCTION_BOMS]:
    "Production BOM snapshot bersifat read-only. Offline → Firebase diblokir karena BOM berdampak ke estimasi material/HPP.",
  [LOCAL_DB_TABLES.PRODUCTION_PAYROLLS]:
    "Production payroll snapshot bersifat read-only. Offline → Firebase diblokir karena payroll final/payment tetap Firebase-primary.",
  [LOCAL_DB_TABLES.PRODUCTION_HPP_SNAPSHOTS]:
    "HPP snapshot bersifat derived read-only. Offline → Firebase diblokir karena HPP final wajib dari payroll final/confirmed/paid.",
});

const DELETE_FIREBASE_BLOCKED_MESSAGE =
  "Delete Firebase diblokir. Ulangi manual sync dengan allowDeletes=true dan deleteConfirmation setelah review scope.";
...
const buildRemoteChangeConflict = ({ queueItem, remoteData }) => {
  if (queueItem.operation !== LOCAL_SYNC_OPERATIONS.UPDATE) return null;

  const baseVersionTime = getComparableVersionTime(queueItem.baseVersion);
  const remoteVersionSource = remoteData?.updatedAt || remoteData?.remoteUpdatedAt || null;
  const remoteVersionTime = getComparableVersionTime(remoteVersionSource);
  const baseRecordFingerprint = queueItem.metadata?.baseRecordFingerprint || "";
  const remoteRecordFingerprint = createSyncRecordFingerprint(remoteData || {});

  if (!baseVersionTime && !baseRecordFingerprint) {
    return {
      conflictType: "missing_base_version_on_update",
      message:
        "Queue update tidak memiliki baseVersion/base fingerprint. Sync update dihentikan agar tidak overwrite Firebase tanpa jejak versi.",
      metadata: {
        baseVersion: describeComparableVersion(queueItem.baseVersion),
        remoteVersion: describeComparableVersion(remoteVersionSource),
      },
    };
  }

  if (baseVersionTime && remoteVersionTime && remoteVersionTime > baseVersionTime) {
    return {
      conflictType: "remote_changed_on_update",
      message:
        "Dokumen Firebase berubah setelah data dipull ke offline. Sync update dihentikan dan perlu resolve conflict manual.",
      metadata: {
        baseVersion: describeComparableVersion(queueItem.baseVersion),
        remoteVersion: describeComparableVersion(remoteVersionSource),
      },
  ...
  if (baseRecordFingerprint && baseRecordFingerprint !== remoteRecordFingerprint) {
    return {
      conflictType: "remote_changed_on_update",
      message:
        "Fingerprint data Firebase berbeda dari base record offline. Sync update dihentikan dan perlu resolve conflict manual.",
      metadata: {
        baseVersion: describeComparableVersion(queueItem.baseVersion),
        remoteVersion: describeComparableVersion(remoteVersionSource),
        baseRecordFingerprint,
        remoteRecordFingerprint,
      },
    };
  }
...
const buildPreviewRows = (queueItems = []) =>
  queueItems.map((item) => {
    const blockedReason = getPreviewBlockedReason(item);
    return {
      queueId: item.id,
...
export const previewFirebaseMasterDataSync = async ({ limit = 50, collectionName = null } = {}) => {
  const queueItems = (await listPendingSyncQueueItems({ collectionName })).slice(0, limit);
...
const syncSingleQueueItem = async (queueItem, { allowDeletes = false } = {}) => {
  ...
  assertNoSensitiveLocalDbPayload(queueItem.payload, "Firebase sync payload");
  assertNoSensitiveLocalDbPayload(queueItem.metadata, "Firebase sync metadata");

  if (queueItem.operation === LOCAL_SYNC_OPERATIONS.DELETE && !allowDeletes) {
    const message = DELETE_FIREBASE_BLOCKED_MESSAGE;
    const updatedQueue = await updateSyncQueueItemStatus(queueItem.id, {
      status: LOCAL_SYNC_STATUSES.FAILED,
  ...
  const remoteChangeConflict = buildRemoteChangeConflict({ queueItem, remoteData });
  if (remoteChangeConflict) {
    await createSyncConflict({
      queueId: queueItem.id,
      collectionName: queueItem.collectionName,
      documentId: queueItem.documentId,
      conflictType: remoteChangeConflict.conflictType,
      localPayload: queueItem.payload,
      remotePayload: remoteData,
      message: remoteChangeConflict.message,
      metadata: remoteChangeConflict.metadata,
    });
    const updatedQueue = await updateSyncQueueItemStatus(queueItem.id, {
      status: LOCAL_SYNC_STATUSES.CONFLICT,
      errorMessage: remoteChangeConflict.message,
      metadataPatch: {
        conflictType: remoteChangeConflict.conflictType,
        remoteVersionGuardedAt: timestamp,
      },
    });
...
export const syncPendingMasterDataToFirebase = async ({
  ...
  if (allowDeletes && deleteConfirmation !== MASTER_DATA_DELETE_SYNC_CONFIRMATION) {
    throw new Error(
      `Delete Firebase dari offline queue butuh confirmation terpisah: ${MASTER_DATA_DELETE_SYNC_CONFIRMATION}`
    );
  }
