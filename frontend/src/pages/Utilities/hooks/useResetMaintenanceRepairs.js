...
const useResetMaintenanceRepairs = ({
  ...
  const handleRepairMasterCodeAudit = useCallback(async () => {
    try {
      ...
      showActionSuccess(result?.message || "Normalisasi kode master selesai.");
      const nextAudit = await getMasterCodeMaintenanceAudit();
      setMasterCodeAudit(nextAudit);
      await loadPreview(false);
    } catch (error) {
      console.error(error);
      showActionError(error?.message || "Gagal menjalankan normalisasi kode master.");
    } finally {
  ...
  const handleRepairProductionMaintenance = useCallback(async () => {
    try {
      ...
      showActionSuccess(result?.message || "Repair varian produksi selesai.");
      const nextAudit = await getProductionVariantMaintenanceAudit();
      setMaintenanceAudit(nextAudit);
      await loadPreview(false);
    } catch (error) {
      console.error(error);
      showActionError(error?.message || "Gagal menjalankan repair varian produksi.");
    } finally {
  ...
  const handleRepairStockAudit = useCallback(async () => {
    try {
      ...
      showActionSuccess(result?.message || "Repair stok umum selesai.");
      const nextAudit = await getInventoryStockMaintenanceAudit();
      setStockAudit(nextAudit);
      await loadPreview(false);
    } catch (error) {
      console.error(error);
      showActionError(error?.message || "Gagal menjalankan repair stok umum.");
    } finally {
  ...
  const handleRepairLogSchema = useCallback(async () => {
    try {
      ...
      showActionSuccess(result?.message || "Repair schema inventory log selesai.");
      const nextAudit = await getInventoryLogSchemaAudit();
      setLogSchemaAudit(nextAudit);
    } catch (error) {
      console.error(error);
      showActionError(error?.message || "Gagal menjalankan repair schema inventory log.");
    } finally {
  ...
  const handleRepairHppReconcileAudit = useCallback(async () => {
    try {
      ...
      console.error(error);
      showActionError(error?.message || "Gagal menjalankan repair HPP output.");
    } finally {
  ...
  const handleRepairPayrollAudit = useCallback(async () => {
    try {
      ...
      showActionSuccess(result?.message || "Repair snapshot payroll selesai.");
      const nextAudit = await getPayrollSnapshotMaintenanceAudit();
      setPayrollAudit(nextAudit);
      await loadPreview(false);
    } catch (error) {
      console.error(error);
      showActionError(error?.message || "Gagal menjalankan repair payroll snapshot.");
    } finally {
  ...
  const handleRepairTransactionVariantAudit = useCallback(async () => {
    try {
      ...
      console.error(error);
      showActionError(error?.message || "Gagal menjalankan repair variant lintas modul.");
    } finally {
  ...
  const handleRepairTransactionSideEffects = useCallback(async ({ actionNote = "" } = {}) => {
    try {
      ...
      showActionSuccess(result?.message || "Repair side-effect transaksi selesai.");
      const nextAudit = await getTransactionSideEffectRepairAudit();
      setTransactionSideEffectAudit(nextAudit);
      await loadPreview(false);
    } catch (error) {
      console.error(error);
      showActionError(error?.message || "Gagal menjalankan repair side-effect transaksi.");
      throw error;
    } finally {
