...
export const calculateSupplierMaterialRestockMetrics = (detail = {}) => ({
  referencePrice: Number(detail.referencePrice || detail.price || 0),
  conversionValue: Number(detail.conversionValue || 1),
});
...
export const getSupplierMaterialDetail = (supplier = {}, materialId = "") => {
  ...
  return materialDetails.find(
    (item) => String(item.materialId || item.rawMaterialId || item.id) === String(materialId)
  ) || null;
};
...
export const doesSupplierProvideMaterial = (supplier = {}, materialId = "") => Boolean(
  getSupplierMaterialDetail(supplier, materialId)
);
