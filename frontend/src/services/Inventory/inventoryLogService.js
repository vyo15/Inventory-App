...
export const resolveInventoryLogReference = (type = "", extraData = {}) => {
  const referenceId = extraData.referenceId
    || extraData.saleId
  ...
  const referenceType = extraData.referenceType
...
export const resolveInventoryLogDisplayReference = (record = {}) => resolveDisplayReference(record, {
  fallback: "",
  extraFields: [
    "sourceRef",
    "referenceCode",
...
export const buildInventoryLogReferenceFields = ({
  ...
  referenceId,
  referenceNumber: referenceNumber || referenceCode || sourceRef || "",
  referenceCode: referenceCode || referenceNumber || sourceRef || "",
  sourceRef: sourceRef || referenceNumber || referenceCode || "",
...
export const buildInventoryLogVariantFields = ({
  ...
  variantKey: variantKey || selectedVariant?.variantKey || "",
  variantLabel: variantLabel || selectedVariant?.variantLabel || selectedVariant?.name || "",
...
export const buildInventoryLogUnitFields = (options = {}) => {
  const normalizedUnit = resolveInventoryStockUnit(options);
  return {
    unit: options.unit || normalizedUnit,
    stockUnit: options.stockUnit || normalizedUnit,
  };
};
