...
export const buildSaleStockBucketKey = (item) => {
  return `${item.collectionName}::${item.itemId}::${item.variantKey || "master"}`;
};

export const fetchSalesRecords = async () => {
  const records = await sqliteTransactionsAdapter.listSales({ limit: 1000 });

  return records.map((item) => ({
    ...item,
    date: item.transactionDate || item.date || item.createdAt || "Tanggal Tidak Tersedia",
  }));
};
...
export const buildSaleIncomePayload = ({ selectedSale, saleId, dateValue = new Date().toISOString() }) => {
  const referenceNumber = selectedSale.saleNumber || selectedSale.code || selectedSale.referenceNumber || saleId;

  return {
    date: dateValue,
    type: "Penjualan",
    incomeNumber: referenceNumber,
    referenceNumber,
    relatedId: saleId,
    description: `Penjualan: ${(selectedSale.items || []).map((item) => item.itemName).join(", ")}`,
    amount: selectedSale.total || selectedSale.totalAmount || 0,
  };
};
...
export const createSaleTransaction = async ({ values, saleItems = [] } = {}) => {
  ...
  return sqliteTransactionsAdapter.commitSale({
    ...values,
    items,
    status: values.status || "Diproses",
    transactionDate: values.date ? dayjs(values.date).format("YYYY-MM-DD") : new Date().toISOString(),
    totalAmount: Number(values.totalAmount ?? values.total ?? 0),
  });
};
