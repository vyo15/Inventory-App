...
- `.gitattributes` dan `.editorconfig` mengunci file `*.js`, `*.jsx`, `*.cjs`, `*.mjs`, `*.css`, `*.json`, dan `*.md` ke LF. Source-hygiene test memindai seluruh extension tersebut agar CRLF/mixed EOL tidak kembali. Normalisasi JSON/Markdown dilakukan sebagai perubahan mechanical tanpa mengubah business logic.
...
- **Frontend orchestration cleanup 2026-07-01:** contract form besar dikelompokkan berdasarkan state/reference/actions, close/reset drawer dipusatkan pada callback parent, dan behavioral test ditambahkan untuk Purchase, Sales, Production Order, Stock Adjustment, User Profile, serta mapper Payroll export. Konfigurasi table/mobile Production Order, BOM, Work Log, Sales, dan Purchase dipindahkan ke helper/list component domain existing. Kalkulasi form Purchase kini memakai helper murni teruji dan satu snapshot biaya deterministik; preview stok dipindahkan ke helper read-only; default item dijaga agar listener refresh tidak mereset varian/konversi yang sedang diisi. `Purchases.jsx` tetap menjadi orchestrator guarded, tetapi sudah di bawah 1.000 baris tanpa memindahkan authority commit atomic dari `purchasesService`/backend.
- **Safety/helper cleanup M8 — 2026-07-02:** path containment dan realpath ancestor dipusatkan di `backend/src/utils/pathSafety.js`; actor audit request dipusatkan di `backend/src/utils/requestActor.js`; normalizer upper/lower, rounded integer, dan timestamp memakai utility canonical. Controller tetap memiliki authority domain dan hanya memakai helper untuk metadata/normalisasi.
- **Numeric/read-model cleanup M8 — 2026-07-02:** adapter Stock Read Model, Finance, Pricing Rules, pricing service, formatter angka, dan variant archive memakai `frontend/src/utils/number/numberNormalization.js`. Nilai legacy non-finite dinetralkan tanpa mengubah rounding, clamp, stock mutation, atau formula pricing. Presentasi saving Cash Out dan Purchases Report memakai satu helper read-only.
- **Report/source hygiene M8 — 2026-07-02:** Sales/Purchases/Profit-Loss berbagi `ReportPeriodFilterSection`; `.githooks/*`, `.gitignore`, `.nvmrc`, `.node-version`, dan `*.env.example` dikunci LF. Verifier menolak `_PATCH_MANIFEST.md`, `_PATCH_DELETE_FILES.txt`, serta folder paket `*changed-files*` agar metadata patch tidak masuk source.
- **Canonical normalization cleanup 2026-07-02:** normalisasi text frontend dipusatkan di `frontend/src/utils/text/textNormalization.js`, sedangkan backend memakai `backend/src/utils/textNormalization.js`. Migrasi mempertahankan dua contract lama secara eksplisit (`normalizeText` untuk nullish-preserving dan `normalizeTruthyText` untuk legacy falsy-as-empty); integer rounding backend memakai `toRoundedInteger`. Source-hygiene test mencegah definisi lokal `safeTrim`, `normalizeText`, dan `toInteger` kembali tersebar.
- **Finance presentation cleanup 2026-07-02:** Cash In dan Cash Out berbagi `CashFlowPageShell` untuk header, summary, filter, table, dan modal form. Create/delete transaction, source classification, delete guard, serta service call tetap dimiliki page domain masing-masing.
- **Purchase characterization 2026-07-02:** subscription reference data dan snapshot form dipisahkan ke hook lokal read-only; OCR, stock-in, expense, supplier verification, dan atomic `createPurchaseTransaction` tetap di page/service. Test helper mencakup offer selection, catalog field snapshot, serta guard subtotal manual.
- **Lint/test hygiene 2026-07-02:** source-runtime test mendeteksi import PascalCase yang dapat terlewat oleh `varsIgnorePattern`; unused import yang terbukti telah dibersihkan. Mock `ProtectedRoute` diisolasi per file agar full suite tidak dipengaruhi mock `useAuth` dari test lain.
...
- **Database Center presentation cleanup:** panel coverage, detail teknis/runtime, dan preview restore dipisahkan menjadi komponen presentational. Handler backup/import/preview/restore, confirm keyword, pre-restore backup, dan service call tetap berada di `OfflineDatabaseCenter.jsx`.
- **Source packaging cleanup:** `gitpush.txt` dipertahankan sebagai compatibility note yang mengarahkan ke `npm run clean:zip`/`clean:zip:ps`; script PowerShell hardcoded lama tidak lagi menjadi jalur pembuatan ZIP.
