...
- Tidak ditemukan database browser arsip adapter khusus `purchases` atau `sales`.
...
- Audit ini static source review dari ZIP frontend, bukan inspeksi data database arsip production.
- rules database arsip/index tidak ada di ZIP dan harus dicek manual di runtime arsip Console jika nanti Purchases/Sales draft atau commit online diubah.
...
- Docs sudah menyatakan supplier belum boleh dipakai write offline. Source Sales sudah punya guard lebih ketat: Sales customer reference dibaca dari runtime arsip-primary lewat `salesCustomerReferenceService`, bukan dari customer local-only.
...
-> database arsip runTransaction
   -> reserve/generate PUR-DDMMYYYY-001
   -> validate supplier dari daftar supplier aktif
   -> validate item product/raw material dari transaksi database arsip snapshot
...
|---|---|---|
| Purchase final | `purchases/{PUR-*}` | runtime arsip-only sampai commit online dirancang |
| Stock masuk | `products` / `raw_materials` via transaksi database arsip | Tidak boleh offline mutation |
...
Purchase final wajib menjaga field berikut saat commit runtime arsip:
...
| `itemId`, `itemName` | Ya | Snapshot item final yang divalidasi ulang dari runtime arsip |
| `variantKey`, `variantLabel`, `stockSourceType` | Wajib jika item bervarian | Mencegah stok masuk ke master/default yang salah |
| `supplierId`, `supplierName` | Ya | Harus berasal dari supplier runtime arsip/resolved supplier aktif |
...
3. Raw material/product bervarian membutuhkan validasi stok/variant shape dari runtime arsip saat commit.
...
-> system re-fetch supplier + item + variant + stock/cost terbaru dari runtime arsip
-> user review perbedaan
-> user klik Commit Purchase
-> createPurchaseTransaction existing berjalan runtime arsip transaction
...
1. Re-fetch supplier dari runtime arsip dan pastikan supplier masih aktif/valid.
2. Re-fetch product/raw material dari runtime arsip transaction snapshot.
...
7. Menandai draft committed hanya setelah transaction runtime arsip sukses.
...
-> load customer via salesCustomerReferenceService (runtime arsip-primary)
-> user isi form
-> buildSaleLine dengan guard availableStock UI cache
-> validateSaleStockAvailability dari runtime arsip
...
| Area | Source of truth aktual | Catatan offline |
|---|---|---|
| Sales final | `sales/{ORD-*}` | runtime arsip-only sampai commit online dirancang |
| Customer reference | `salesCustomerReferenceService` → `customersService` runtime arsip-primary | Customer local-only tidak boleh dipakai transaksi |
...
| `salesChannel` | Ya | Channel transaksi; `Offline` di sini berarti channel toko, bukan database browser arsip mode |
| `status` | Ya | `Diproses`, `Dikirim`, atau `Selesai` |
| `customerId`, `customerName`, customer snapshot | Opsional/bersyarat | Harus dari runtime arsip-primary bila dipilih |
...
3. Customer local-only belum boleh dipakai sebagai foreign reference transaksi runtime arsip.
...
- Menggunakan customer local-only untuk sales final runtime arsip.
...
Saat online
-> re-fetch customer runtime arsip-primary jika customerId terisi
...
-> createSaleTransaction existing berjalan runtime arsip transaction
-> draft ditandai committed setelah sukses
```
...
| Customer/supplier local-only dipakai transaksi | Foreign reference runtime arsip putus |
...
- Draft tidak boleh masuk `sync queue arsip` final otomatis.
...
- Tidak ada schema storage browser arsip/database arsip baru.
- Tidak ada route/menu/role guard baru.
- Tidak ada mutasi stock/finance/report offline.
- Keputusan awal tetap: **Purchases/Sales final mutation tetap runtime arsip-only**.
...
- [ ] Sales customer dropdown tetap runtime arsip-primary; customer local-only tidak muncul/dipakai transaksi.
