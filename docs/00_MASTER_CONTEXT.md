...
- Backup resmi SQLite memakai satu file `.imsbackup` self-contained berisi database, manifest, checksum, dan README internal; file `.manifest.json` terpisah tidak dibuat lagi. Snapshot, lifecycle daily/monthly/retention, dan restore file swap memegang database coordinator eksklusif agar tidak berjalan bersamaan dengan request operasional. Saat startup backend menjalankan lifecycle satu kali, lalu scheduler aktual berjalan setiap 1 jam sampai shutdown dan dihentikan secara terkontrol. Fase monthly, retention, dan daily terisolasi agar satu error tidak membatalkan fase lain. Status scheduler, run terakhir/berikutnya, error terakhir, dan ringkasan hasil dipublikasikan melalui maintenance status. Struktur folder aktif hanya `daily/`, `monthly/`, dan `manual/`. Daily dibuat otomatis maksimal satu per hari dan disimpan 60 hari. Monthly dibuat otomatis dari daily verified terakhir setiap bulan dan disimpan maksimal 12 bulan. Backup manual, import, pre-update, pre-reset, pre-repair, dan pre-restore disimpan pada folder `manual/` serta tidak dihapus otomatis. Pembuatan, promosi monthly, import, dan ekstraksi backup melakukan preflight ruang disk. Paket memakai ZIP klasik tanpa ZIP64 dan menolak entry database 4 GB atau lebih secara eksplisit. Restore tetap full replace guarded, bukan merge. Backup legacy `.imsbak.zip`, sidecar manifest lama, dan record backup legacy yang sudah berada di storage resmi tetap dibaca sebagai kompatibilitas. File dari lokasi luar tidak pernah dipakai, didownload, dipromosikan, direstore, atau dihapus langsung berdasarkan path registry; file tersebut wajib diimport/copy melalui Maintenance Center ke folder `manual/` terlebih dahulu. Backup `pre-restore` dan backup sumber restore dipastikan tercatat ulang setelah restore agar rollback dan traceability tetap terlihat di daftar backup.
...
- **Protected permanen:** stock, inventory log, transactions, finance, production, payroll, backup/restore history, dan audit log tidak masuk allowlist purge.
- **Anti-regression:** strict SQLite audit memeriksa route realtime dan menolak hard-delete runtime di luar file Maintenance guarded.

## Lab Pengujian Sandbox (2026-06-28)
- Lab Pengujian dipisahkan dari Maintenance Center dan hanya untuk Administrator.
- Runtime wajib memakai purpose `sandbox`, database terpisah, dan backup storage terpisah; database operasional ditolak oleh backend.
