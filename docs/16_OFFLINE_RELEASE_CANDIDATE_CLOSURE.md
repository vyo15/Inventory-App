...
Status: **AKTIF / RELEASE CANDIDATE / MANUAL QA REQUIRED**.
...
- `sync_queue` hanya boleh untuk `categories` dan `customers`.
...
- [ ] Snapshot read-only tidak pernah masuk Offline → Firebase.
- [ ] Purchase/Sales/Returns/Finance/Production/Payroll/HPP tetap Firebase-primary.
- [ ] Report/Finance snapshot tidak dianggap laporan final jika belum refresh dari Firebase.
...
- Offline → Firebase update akan masuk `sync_conflicts` jika Firebase berubah setelah data dipull ke local, bukan overwrite otomatis.
...
- `firestore.rules` masih perlu audit production rules terpisah. Jangan ubah field-level/module-level rules di batch offline closure karena bisa memutus flow transaksi.
- `src/firebase.js` masih memakai public Firebase Web config hardcoded. Ini bukan secret server, tetapi env-based config boleh menjadi cleanup terpisah setelah deployment workflow disetujui.
...
- Pull `categories/customers` dari Firebase, edit record yang sama di Firebase/device lain, lalu sync offline update; hasil yang benar adalah `sync_conflicts`, bukan overwrite.
