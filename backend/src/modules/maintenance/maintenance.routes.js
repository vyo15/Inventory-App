...
const getUniqueBackupPath = (targetDir, filename) => {
  ...
  const extension = SUPPORTED_BACKUP_FILE_SUFFIXES.find((suffix) => candidateName.toLowerCase().endsWith(suffix)) || path.extname(candidateName);
...
const normalizeExportRow = (row = {}, tableConfig = {}) => {
  const payload = parseJsonField(row.payload_json, null);
  const normalized = {
    ...row,
...
router.get("/master-data-export", requireLocalAuth, requireLocalAdministrator, async (req, res, next) => {
  try {
...
router.get("/backups/:filename/download", requireLocalAuth, requireLocalAdministrator, async (req, res, next) => {
  try {
    const requestedFilename = normalizeBackupFilename(req.params.filename || "");
...
router.post(
  ...
  express.raw({ type: ["application/octet-stream", "application/imsbackup", "application/x-imsbackup"], limit: IMPORT_BACKUP_MAX_BYTES }),
  async (req, res, next) => {
    let importedPath = "";
    try {
      const backupBuffer = Buffer.isBuffer(req.body) ? req.body : null;
      ...
      const rawFilename = decodeImportFilename(req.headers["x-ims-backup-filename"] || req.query?.filename || "");
      ...
      if (!safeSourceName || !isSupportedBackupPackageName(safeSourceName)) {
        throw createHttpError("Format file backup tidak didukung. Gunakan File Backup IMS (.imsbackup) atau backup legacy .imsbak.zip.", 400, "BACKUP_IMPORT_UNSUPPORTED_FORMAT");
      }
