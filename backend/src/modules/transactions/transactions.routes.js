...
router.post("/returns/commit", requireLocalAuth, requireLocalAdministrator, async (req, res, next) => {
  ...
  try {
    ...
    const body = await buildSaleReturnValidation(db, req.body || {});
    ...
    await db.run("COMMIT");
    return success(res, "Retur berhasil disimpan dari transaksi Sales dan stok dipulihkan.", { ...transactionRecord, mutationResults, financeResult }, undefined, 201);
  } catch (error) {
    await db.run("ROLLBACK").catch(() => {});
    if (error.statusCode) {
...
router.use("/returns", createSqliteJsonRecordRouter({
  ...
  requiredName: false,
  orderBy: "transaction_date DESC, updated_at DESC",
  protectedWriteNote: "Returns aktif wajib terkait transaksi Sales. Direct write generic diblokir agar stok kembali tidak bisa dibuat tanpa relasi sales.",
  allowDirectCreate: false,
  allowDirectUpdate: false,
  allowDirectDelete: false,
  blockedWriteMessage: "Return wajib lewat POST /api/transactions/returns/commit dan harus terkait Sales agar qty retur, stok kembali, audit log, dan transaction record tetap terkunci.",
}));
