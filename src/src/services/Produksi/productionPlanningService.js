...
// =====================================================
// ACTIVE - helper priority plan Dashboard.
// Fungsi:
// - menyusun maksimal 3 planning yang perlu dikejar berdasarkan overdue, deadline,
//   progress terkecil, dan sisa target terbesar;
// - hanya memakai hasil progress existing dari getAllProductionPlans.
// Hubungan flow aplikasi:
// - Dashboard membaca output ini untuk section Fokus Target Produksi;
// - tidak menyimpan field baru dan tidak mengubah schema Firestore.
// Status:
// - aktif; read-only dan bukan logic stok/payroll/HPP.
// =====================================================
const buildDashboardPriorityPlans = (plans = []) => {
  return plans
    .filter((plan) => {
      const status = safeTrim(plan.status).toLowerCase();
      if (!["active", "overdue"].includes(status)) return false;
      return toNumber(plan.remainingQty) > 0;
    })
    .sort((a, b) => {
      const statusRankA = a.status === "overdue" ? 0 : 1;
      const statusRankB = b.status === "overdue" ? 0 : 1;
      if (statusRankA !== statusRankB) return statusRankA - statusRankB;

      const dueA = toComparableDate(a.dueDate) || Number.MAX_SAFE_INTEGER;
      const dueB = toComparableDate(b.dueDate) || Number.MAX_SAFE_INTEGER;
      if (dueA !== dueB) return dueA - dueB;

      const progressDiff = toNumber(a.progressPercent) - toNumber(b.progressPercent);
      if (progressDiff !== 0) return progressDiff;

      return toNumber(b.remainingQty) - toNumber(a.remainingQty);
    })
    .slice(0, 3)
    .map((plan) => ({
      id: plan.id,
      ...
      title: plan.title || "",
      targetItemName: plan.targetItemName || plan.title || "Target produksi",
      targetVariantLabel: plan.targetVariantLabel || "",
      ...
      linkedProductionOrderIds: Array.isArray(plan.linkedProductionOrderIds)
        ? plan.linkedProductionOrderIds
        : [],
    }));
};
...
// - menghitung overdue dan planning kurang target;
// - menambahkan priorityPlans read-only untuk Dashboard Focus.
// Status:
...
export const getProductionPlanningDashboardSummary = async () => {
  ...
  const openPlans = activePlans.filter(
  ...
  return {
    ...
    activePlanningCount: openPlans.length,
    ...
    priorityPlans: buildDashboardPriorityPlans(activePlans),
  };
};
